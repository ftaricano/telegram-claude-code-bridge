...
def test_security_relaxation_settings_defaults_and_overrides(monkeypatch):
    ...
    monkeypatch.delenv("DISABLE_SECURITY_PATTERNS", raising=False)
...
def test_voice_provider_local_requires_no_api_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("WHISPER_CPP_MODEL_PATH", "")
    monkeypatch.setenv("WHISPER_CPP_BINARY_PATH", "")
    project_dir = tmp_path / "projects"
    project_dir.mkdir()
