...
class TestSecurityValidator:
    ...
    def test_boundary_only_check(self, validator, temp_approved_dir):
        """check_patterns=False skips the patterns but keeps the boundary."""
        for name in ("app/blog/[...slug]/page.tsx", "app/routes/posts.$postId.tsx"):
            assert validator.validate_path(name, temp_approved_dir)[0] is False
            valid, path, error = validator.validate_path(
                name, temp_approved_dir, check_patterns=False
            )
            assert valid is True
            assert path == (temp_approved_dir / name).resolve()
            assert error is None

        for escape in ("../outside", "app/blog/[...slug]/../../../../outside"):
            valid, _path, error = validator.validate_path(
                escape, temp_approved_dir, check_patterns=False
            )
            assert valid is False
