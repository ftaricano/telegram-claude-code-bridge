...
class TestStopButtonOnProgress:
    ...
    async def test_agentic_text_starts_typing_in_current_forum_topic(
        self, orchestrator, settings
    ):
        """Initial sendChatAction must use the triggering message_thread_id."""
        user_id = 42
        mock_response = ClaudeResponse(
            content="Done",
            session_id="s1",
        ...
        update.message = AsyncMock()
        update.message.message_id = 1
        update.message.message_thread_id = 7010
        update.message.text = "test"
        update.message.chat = AsyncMock()
        update.message.chat.send_action = AsyncMock()
        update.effective_message = update.message
        ...
        update.message.chat.send_action.assert_any_call(
            "typing", message_thread_id=7010
        )
        mock_hb.assert_called_once()
        assert mock_hb.call_args.kwargs["message_thread_id"] == 7010
