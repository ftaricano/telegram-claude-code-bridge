...
class TestTypingHeartbeat:
    ...
    async def test_heartbeat_starts_immediately_and_keeps_sub_3s_gap(
        ...
        """Typing starts immediately and remains below Telegram's visible gap budget."""
        chat = AsyncMock()
        sent_at: list[float] = []
    ...
    async def test_heartbeat_passes_message_thread_id_for_forum_topics(
        ...
        """sendChatAction must target the active forum topic."""
        chat = AsyncMock()
        chat.send_action = AsyncMock()
    ...
    async def test_heartbeat_survives_long_sdk_reentries_with_tool_calls(
        ...
        """Acceptance: heartbeat keeps cadence during a long SDK turn that
