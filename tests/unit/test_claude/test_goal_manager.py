...
async def test_evaluator_loads_project_settings_only_by_default(storage):
    """The goal evaluator does not load user settings unless asked to."""
    assert GoalManager(storage.goals).setting_sources == ["project"]
    manager = GoalManager(storage.goals, setting_sources=["project", "user"])
    assert manager.setting_sources == ["project", "user"]
