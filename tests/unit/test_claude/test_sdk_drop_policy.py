"""Drop policy tests for Claude SDK backpressure."""
...
async def test_drop_policy_drops_only_stream_deltas_when_queue_is_full(
    tmp_path, monkeypatch
):
    """Full queue drops deltas but waits for ToolUse/TextBlock assistant messages."""
    ...
    final_text = AssistantMessage(
        content=[TextBlock(text="final text")],
        model="claude-sonnet-4-20250514",
    )
    messages = [
        tool_message,
        *[_make_stream_delta(str(i), i) for i in range(10)],
        final_text,
    ...
    assert any(
        update.type == "assistant" and update.content == "final text"
