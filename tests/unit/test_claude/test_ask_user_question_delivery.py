"""Regression tests: AskUserQuestion must be user-visible.
...
async def test_text_plus_ask_user_question_tool_is_delivered_as_numbered_prompt(
    ...
    rich_text = AssistantMessage(
        content=[TextBlock(text="Minha recomendação é atacar a Opção A primeiro.")],
    ...
    assert "1) Opção A" in response.content
    assert "2) Opção B" in response.content
    assert "3) Opção C" in response.content
