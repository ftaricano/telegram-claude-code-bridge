...
Telegram bot providing remote access to Claude Code. Python 3.11+, built with uv (PEP 621 + hatchling), using `python-telegram-bot` for Telegram and `claude-agent-sdk` for Claude Code integration.

## Commands

```bash
make dev              # Install all deps (including dev) — runs uv sync
make install          # Production deps only — runs uv sync --no-dev
...
Sessions auto-resume: per Telegram topic, persisted in SQLite table
`topic_sessions` keyed by `(chat_id, message_thread_id)`. `message_thread_id=1`
is the General-topic sentinel. Session IDs come from Claude's `ResultMessage`;
the bridge does not generate them locally. `/new` deletes only the current
topic row, topic close marks `is_active=0`, and the next message reactivates the
same row. Agentic mode uses fixed `APPROVED_DIRECTORY`; `/repo` is deprecated.
...
- `src/storage/` -- SQLite via aiosqlite, repository pattern (users, sessions, messages, tool_usage, audit_log, cost_tracking, project_threads, topic_sessions)
...
Settings loaded from environment variables via Pydantic Settings. Required: `TELEGRAM_BOT_TOKEN`, `TELEGRAM_BOT_USERNAME`, `APPROVED_DIRECTORY`. Key optional: `ALLOWED_USERS` (comma-separated Telegram IDs), `ENABLE_MCP`, `MCP_CONFIG_PATH`. Direct Anthropic API keys are intentionally unsupported; use Claude CLI/OAuth.

Agentic platform settings: `AGENTIC_MODE` (default true), `ENABLE_API_SERVER`, `API_SERVER_PORT` (default 8080), `GITHUB_WEBHOOK_SECRET`, `WEBHOOK_API_SECRET`, `ENABLE_SCHEDULER`, `NOTIFICATION_CHAT_IDS`, `MEDIA_GROUP_WINDOW_SECONDS` (default `1.8`, Telegram photo album debounce window).
...
Agentic mode commands: `/start`, `/new`, `/status`, `/goal`, `/verbose`, `/context`, `/compact`. `/repo` remains registered only as a deprecated compatibility handler and must not be added to the Telegram command menu. If `ENABLE_PROJECT_THREADS=true`: `/sync_threads`. To add a new command:
