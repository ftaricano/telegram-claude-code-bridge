...
def _proxy_userinfo(proxy_url: str) -> str:
    """Return the ``user[:password]`` part of a proxy URL, or ``""``.

    Split by hand rather than with ``urlparse`` so that a value without a
    scheme (``user:pass@host:port``) is covered as well.
    """
    rest = proxy_url.partition("://")[2] or proxy_url
    return rest.rpartition("@")[0]


def _mask_userinfo(userinfo: str) -> str:
    """Keep the user name, mask the password or a user-only token."""
    user, colon, _password = userinfo.partition(":")
    return f"{user}:***" if colon else "***"


def _redact_proxy_url(proxy_url: str) -> str:
    """Mask the credentials in a proxy URL so it is safe to log.

    ``HTTPS_PROXY``/``HTTP_PROXY`` commonly carry credentials as
    ``scheme://user:pass@host:port``, and some proxies take a token as the
    user name. Replace the password (or the token) with ``***`` while keeping
    the scheme, user and host readable.
    """
    try:
        urlparse(proxy_url)
    except ValueError:
    ...
    userinfo = _proxy_userinfo(proxy_url)
    if not userinfo:
        return proxy_url
    return proxy_url.replace(f"{userinfo}@", f"{_mask_userinfo(userinfo)}@", 1)
...
class ClaudeCodeBot:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            # httpx quotes a rejected proxy URL, credentials included.
            userinfo = _proxy_userinfo(proxy_url or "")
            if not userinfo or userinfo not in str(e):
                raise
            raise ValueError(
                str(e).replace(userinfo, _mask_userinfo(userinfo))
            ) from None
