...
class FileHandler:
            ...
            if archive_path.suffix == ".zip":
                with zipfile.ZipFile(archive_path) as zf:
                    ...
                    for file_info in zf.filelist:
                        ...
                        target_path = self._safe_archive_target(extract_dir, file_path)
                ...
                with tarfile.open(archive_path) as tf:
                    ...
                    for member in tf.getmembers():
                        # Never extract links or special files from user archives.
                        if not (member.isdir() or member.isfile()):
                            continue

                        target_path = self._safe_archive_target(
                            extract_dir, Path(member.name)
                        )
                        if target_path is None:
                            continue
    ...
    def _safe_archive_target(self, extract_dir: Path, member_path: Path) -> Path | None:
        """Return a safe extraction target or None when the member escapes."""
