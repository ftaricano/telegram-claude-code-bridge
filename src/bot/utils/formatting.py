...
class ResponseFormatter:
    ...
    def format_claude_response(
        ...
        malformed_pattern = self._detect_malformed_markdown(text)
        if malformed_pattern:
            logger.warning(
                "markdown_fallback",
                pattern_detected=malformed_pattern,
                original_length=len(text),
                fallback_strategy="raw_text_no_parse_mode",
            )
            messages = self._split_raw_text(text)
        ...
        if not messages:
            # Surface this case in logs — historically the placeholder
            ...
            # If original text is non-empty, formatting ate the
            # content (e.g. markdown table → HTML). Fall back to raw text
            # so the user actually receives the response instead of a
            # useless placeholder.
            if text and text.strip():
                logger.warning(
                    "format_claude_response_raw_text_fallback",
                    original_text_len=len(text),
                )
                raw = text[: self.max_message_length]
                return [FormattedMessage(raw, parse_mode=None)]
            return [FormattedMessage("<i>(No content to display)</i>")]

        return messages

    def _detect_malformed_markdown(self, text: str) -> Optional[str]:
        """Detect lightweight markdown patterns that can break Telegram parsing."""
        if text.count("```") % 2 == 1:
            return "unbalanced_fence"

        text_without_code = re.sub(r"```.*?```", "", text, flags=re.DOTALL)
        if text_without_code.count("**") % 2 == 1:
            return "unbalanced_bold"
        if re.search(r"(?<=\w)_(?=\w)", text_without_code):
            return "midword_underscore"
        return None

    def _split_raw_text(self, text: str) -> List[FormattedMessage]:
        """Split raw fallback text with parse_mode disabled."""
