...
def _reply_context_for_prompt(message) -> Optional[str]:
    ...
    if media_type == "message" and len(details) == 1:
        return None
    return (
        "Replied-to Telegram message has no text/caption (" + ", ".join(details) + ")."
    )
...
async def _format_progress_update(update_obj) -> Optional[str]:
    ...
    if update_obj.type == "tool_result":
        ...
        tool_names = [
            name for name in update_obj.get_tool_names() if name != "AskUserQuestion"
        ]
        if tool_names:
...
async def handle_text_message(
    ...
    try:
        ...
        # Structured delivery log so silent "message vanished"
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not voice_handler:
        if settings.voice_provider == "local":
            ...
            await update.message.reply_text(
                ...
                '<code>pip install "tapagent-telegram[voice]"</code>.',
                parse_mode="HTML",
            )
        return
    ...
    try:
        ...
        reply_context = _reply_context_for_prompt(update.message)
