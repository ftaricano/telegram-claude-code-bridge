...
class Settings(BaseSettings):
    ...
    claude_max_turns: Optional[int] = Field(
        DEFAULT_CLAUDE_MAX_TURNS,
        description="Max conversation turns (None = sem limite)",
    )
    claude_timeout_seconds: int = Field(
        DEFAULT_CLAUDE_TIMEOUT_SECONDS, description="Claude timeout em segundos"
    )
    ...
    claude_load_user_settings: bool = Field(
        False,
        description=(
            "Also load Claude Code user settings (~/.claude/settings.json). "
            "Off by default: only the project settings are loaded"
        ),
    )
    ...
    sandbox_excluded_commands: Optional[List[str]] = Field(
        ...
        description=(
            "Commands Claude Code runs outside the sandbox. A command line that "
            "contains one of them runs outside the sandbox as a whole; see "
            "SECURITY.md before adding any"
        ),
    )
    ...
    media_group_window_seconds: float = Field(
        1.8,
        description="Seconds to wait for Telegram media-group photos before batching",
        ge=0.0,
    )
    ...
    stream_draft_interval: float = Field(
        0.3,
        description="Minimum seconds between draft updates (0.1-10.0)",
        ge=0.1,
        le=10.0,
    )
    ...
    @property
    def claude_setting_sources(self) -> List[Literal["user", "project", "local"]]:
        """Claude Code setting sources handed to the SDK."""
        if self.claude_load_user_settings:
            return ["project", "user"]
        return ["project"]
