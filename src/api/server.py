...
def create_api_app(
    ...
    app = FastAPI(
        title="TapAgent for Telegram - Webhook API",
        version="0.1.0",
    ...
    async def metrics() -> Dict[str, int]:
        if storage is not None and getattr(storage, "durability", None) is not None:
            return await storage.durability.collect_metrics()
        if db_manager is not None:
            from ..storage.repositories import DurabilityRepository

            repo = DurabilityRepository(db_manager)
            return await repo.collect_metrics()
        return {
            "queue_depth_current": 0,
            "queue_depth_max": 0,
            "dropped_count_last_1h": 0,
            "dropped_count_last_24h": 0,
            "worker_lag_ms_max": 0,
