...
# File tools that write. They may never target Claude Code settings, hooks or
# MCP configuration, even inside the approved directory.
WRITE_FILE_TOOLS = frozenset(
    {"Write", "Edit", "MultiEdit", "NotebookEdit", "create_file", "edit_file"}
)

# File tools that only read.
READ_FILE_TOOLS = FILE_TOOLS - WRITE_FILE_TOOLS

# Tools whose command the can_use_tool callback checks for directory escapes.
BASH_TOOLS = frozenset({"Bash", "bash", "shell"})

# Every tool the callback guards. These must be kept out of the
# ``allowed_tools`` list handed to the SDK: the CLI resolves allow rules before
# sending a permission request, so a tool named in ``allowed_tools`` is
# pre-approved and never reaches ``can_use_tool``, leaving the checks inert.
...
def _tool_matches(tool_name: str, rules: Iterable[str]) -> bool:
    """Return True if ``tool_name`` is covered by one of the tool ``rules``.

    Rules are exact tool names. As in Claude Code permission rules,
    ``mcp__<server>`` and ``mcp__<server>__*`` also cover every tool that
    MCP server exposes.
    """
    for rule in rules:
        if rule == tool_name:
        ...
        if rule.startswith("mcp__"):
            server = rule[:-3] if rule.endswith("__*") else rule
            if server.count("__") == 1 and tool_name.startswith(server + "__"):
                return True
    return False
...
def _make_can_use_tool_callback(
    ...
    read_path_patterns: bool = True,
) -> Any:
    """Create a can_use_tool callback for SDK-level tool permission validation.

    The callback runs *before* the SDK executes a tool that was not
    pre-approved. It denies tools outside ``allowed_tools`` (when a non-empty
    allowlist is given) and tools in ``disallowed_tools``, then validates file
    path boundaries and bash directory boundaries. ``sandbox_excluded_commands``
    are the commands the CLI runs outside the sandbox; a Bash line that joins
    one of them with other commands is denied. With ``read_path_patterns=False``
    the paths of the read-only file tools are checked for the directory boundary
    only, without the path validator's ``DANGEROUS_PATTERNS``.
    """
    ...
    async def can_use_tool(
        ...
        if disallowed_tools and _tool_matches(tool_name, disallowed_tools):
            logger.warning("can_use_tool denied disallowed tool", tool_name=tool_name)
            return PermissionResultDeny(
                message=f"Tool '{tool_name}' is disallowed by configuration"
            )
        if allowed_tools and not _tool_matches(tool_name, allowed_tools):
            logger.warning(
                "can_use_tool denied tool outside allowlist", tool_name=tool_name
            )
            return PermissionResultDeny(
                message=(
                    f"Tool '{tool_name}' is not allowed by configuration "
        ...
        if tool_name in FILE_TOOLS:
            ...
            if file_path:
                # An allow rule written to the agent settings would pre-approve
                # tools and switch these checks off for the next session.
                if tool_name in WRITE_FILE_TOOLS and is_agent_config_path(
                    file_path, working_directory
                ):
                    logger.warning(
                        "can_use_tool denied write to agent configuration",
                        tool_name=tool_name,
                        file_path=file_path,
                    )
                    return PermissionResultDeny(
                        message=(
                            "Writing Claude Code settings, hooks or MCP "
                            "configuration is not allowed"
                        )
                    )

                # A write through a hard link changes every other name of the
                # file, which the path checks below cannot see.
                if tool_name in WRITE_FILE_TOOLS and is_hard_linked_file(
                    file_path, working_directory
                ):
                    logger.warning(
                        "can_use_tool denied write to hard-linked file",
                        tool_name=tool_name,
                        file_path=file_path,
                    )
                    return PermissionResultDeny(
                        message="Writing a file that has other hard links is not allowed"
                    )
                ...
                # The CLI expands a leading "~", so validate the expanded path.
                ...
                valid, _resolved, error = security_validator.validate_path(
                    ...
                    working_directory,
                    check_patterns=(
                        read_path_patterns or tool_name not in READ_FILE_TOOLS
                    ),
                )
                if not valid:
                    logger.warning(
        ...
        if tool_name in BASH_TOOLS:
            # Outside the sandbox only the static check below would stand
            # between the command and the filesystem.
            if tool_input.get("dangerouslyDisableSandbox"):
                logger.warning(
                    "can_use_tool denied unsandboxed bash command",
                    tool_name=tool_name,
                )
                return PermissionResultDeny(
                    message="Running Bash outside the sandbox is not allowed"
                )
            command = tool_input.get("command", "")
            # The CLI runs the whole line outside the sandbox when any of its
            # commands is excluded, so the rest of the line would escape too.
            excluded = (
                find_unsandboxed_command_chain(command, sandbox_excluded_commands)
                if command and sandbox_excluded_commands
                else None
            )
            if excluded:
                logger.warning(
                    "can_use_tool denied bash line with a sandbox-excluded command",
                    tool_name=tool_name,
                    command=command,
                ...
                return PermissionResultDeny(
                    message=(
                        f"'{excluded}' is in SANDBOX_EXCLUDED_COMMANDS, which runs "
                        "the whole command line outside the sandbox. Run it on its "
                        "own, without other commands."
                    )
                )
            if command:
...
def _make_boundary_pre_tool_use_hook(can_use_tool: Any) -> Any:
    """Run the ``can_use_tool`` checks as a PreToolUse hook on the guarded tools.

    ``can_use_tool`` only runs when the CLI asks for permission. A subagent
    whose definition sets ``permissionMode: bypassPermissions`` never asks,
    and neither does a tool that a settings allow rule pre-approves. PreToolUse
    hooks run on every tool call, subagents included, so the checks still apply.

    The hook only denies. When the checks pass it returns no decision and the
    CLI resolves permissions as before, asking ``can_use_tool`` where it would
    have; returning "allow" here would skip that request.

    The hook also sees the reads inside the working directory that the CLI
    allows without asking, so ``can_use_tool`` should be built with
    ``read_path_patterns=False``: a read path is then only checked for the
    directory boundary, and file names such as ``[...slug]`` or ``$id`` pass.
    """
    ...
    async def hook(
        ...
        result = await can_use_tool(
            tool_name, hook_input.get("tool_input") or {}, ToolPermissionContext()
        )
...
class ClaudeSDKManager:
    ...
    async def execute_command(
        ...
        try:
            ...
            # The can_use_tool callback below only runs when the CLI sends a
            # permission request, and the CLI resolves allow rules first. A
            # guarded tool left in allowed_tools is pre-approved and its
            # boundary check never runs, so route those tools to the callback.
            boundary_checks_active = self.security_validator is not None
            if boundary_checks_active:
                gated = [t for t in sdk_allowed_tools if t in GUARDED_TOOLS]
                if gated:
                    sdk_allowed_tools = [
                        t for t in sdk_allowed_tools if t not in GUARDED_TOOLS
                    ]
                    logger.debug(
            ...
            options = ClaudeAgentOptions(
                ...
                sandbox={
                    ...
                    # Auto-approving sandboxed Bash would skip the permission
                    # request the bash boundary check depends on.
                    "autoAllowBashIfSandboxed": not boundary_checks_active,
                    # Keep Bash inside the sandbox: a command that asks to run
                    # unsandboxed would only face the static boundary check.
                    "allowUnsandboxedCommands": not boundary_checks_active,
                    "excludedCommands": self.config.sandbox_excluded_commands or [],
                },
                system_prompt=base_prompt,
                setting_sources=self.config.claude_setting_sources,
                # Without --permission-mode the CLI takes permissions.defaultMode
                # from the loaded settings, and bypassPermissions there approves
                # every tool without asking can_use_tool. Pin "default" while
                # the boundary checks are on; the flag wins over the settings.
                permission_mode="default" if boundary_checks_active else None,
            ...
            if self.security_validator:
                callback_kwargs: Dict[str, Any] = dict(
                    ...
                    allowed_tools=(
                        None
                        if self.config.disable_tool_validation
                        else self.config.claude_allowed_tools
                    ),
                    disallowed_tools=(
                        None
                        if self.config.disable_tool_validation
                        else self.config.claude_disallowed_tools
                    ),
                    sandbox_excluded_commands=(
                        self.config.sandbox_excluded_commands
                        if self.config.sandbox_enabled
                        else None
                    ),
                )
                options.can_use_tool = _make_can_use_tool_callback(**callback_kwargs)
                # The same checks as a hook, for tool calls that never reach
                # can_use_tool (subagents with their own permissionMode). Reads
                # are checked for the directory boundary only.
                options.hooks = options.hooks or {}
                options.hooks.setdefault("PreToolUse", []).append(
                    HookMatcher(
                        matcher="|".join(sorted(GUARDED_TOOLS)),
                        hooks=[
                            _make_boundary_pre_tool_use_hook(
                                _make_can_use_tool_callback(
                                    **callback_kwargs, read_path_patterns=False
                                )
                            )
                        ],
                    )
            ...
            # (avoid the TASK_COMPLETED_MSG fallback when Claude
            ...
            if result_content is not None and str(result_content).strip():
                ...
                # AssistantMessage with text). Without this, the stream
                # surfaces
