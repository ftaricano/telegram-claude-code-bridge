...
This document provides comprehensive information about configuring TapAgent for Telegram.
...
# Disable Claude tool allowlist/disallowlist checks (default: false)
...
# OS-level sandbox for Bash (default: true)
SANDBOX_ENABLED=true

# Commands Claude Code runs outside the sandbox, as a JSON list (default: [])
# WARNING: a command line that contains an excluded command runs outside the
# sandbox as a whole, not only that command. make, npm, docker and git run code
# from files the agent can write, so any entry removes the Bash boundary.
# Matching: "git" matches only the bare command, "git:*" matches git with any
# arguments. See SECURITY.md before adding an entry.
...
# Also load Claude Code user settings (~/.claude/settings.json). Default: false,
# only the project settings are loaded
# While the tool boundary checks are on, the bridge starts Claude Code in the
# "default" permission mode, so permissions.defaultMode in the loaded settings
# (for example bypassPermissions) does not turn the checks off.
CLAUDE_LOAD_USER_SETTINGS=false
```
