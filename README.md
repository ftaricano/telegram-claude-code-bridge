...
[![CI](https://github.com/ftaricano/tapagent-telegram/actions/workflows/ci.yml/badge.svg)](https://github.com/ftaricano/tapagent-telegram/actions/workflows/ci.yml)

Chat with Claude Code from Telegram. TapAgent for Telegram connects Telegram
chats and forum topics to the Claude Code CLI. It is built for remote
development workflows where each Telegram topic can keep its own Claude
session, durable delivery state, project context, and operator controls.
...
```bash
git clone https://github.com/ftaricano/tapagent-telegram.git
cd tapagent-telegram
make dev
```
...
- Claude file operations are constrained to `APPROVED_DIRECTORY`, and Claude cannot write
  Claude Code configuration (`.claude/`, `.mcp.json`). See the limits in
  [SECURITY.md](SECURITY.md#tool-call-enforcement).
...
.env
config/mcp.json
config/projects.*.yaml   (except config/projects.example.yaml)
data/
logs/
htmlcov/
*.db
...
[RichardAtCT/claude-code-telegram](https://github.com/RichardAtCT/claude-code-telegram),
originally authored by Richard Atkinson. This fork is maintained by
ftaricano and includes substantial changes for topic-scoped Claude Code
